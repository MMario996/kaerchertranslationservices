 ...
 * Spalten: Email | Subtabs (z.B. "users,templates,dashboard")
 * "dashboard" schaltet den Analytics-Reiter "Dashboard" frei (kein Admin-Subtab).
 */
var ADMIN_LIGHT_SHEET_NAME_ = "Whitelist_AdminLight";
var ADMIN_LIGHT_VALID_SUBTABS_ = ["users", "templates", "custompages", "pivot", "tests", "dashboard"];
...
function apiAddAdminLightUser(email, subtabs) {
  ...
  if (!cleanSubtabs.length) return { success: false, error: "Mindestens einen Bereich (users/templates/custompages/pivot/tests/dashboard) angeben." };
  try {
    const ss = openAccessSS_();
